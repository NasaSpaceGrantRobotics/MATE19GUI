...
string convertTo3BitHex(int decimal) {
    ...
    converted = converted + cfirstChar + csecondChar + cthirdChar;
    return converted;
}

int main(int argc, char **argv) {
	//Open i2c bus    
	if((file_i2c = open(filename, O_RDWR)) < 0) {printf("\tFailed to open the i2c bus\n");}
...
void arrayCallback(const std_msgs::UInt16MultiArray::ConstPtr& array){
	...
	char hexVals[32];
	string sinput1; //contains converted hex values
	for (int i =0; i<8; i++){
